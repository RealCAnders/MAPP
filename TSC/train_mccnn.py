...
def create_mc_cnn_for_1d(input_len):

  # Define three 1D-Convolutions with Pooling after each input_channel
  in_layer = tf.keras.Input((input_len,1))
  c_conv = tf.keras.layers.Conv1D(8, 5, use_bias=False)(in_layer)
  c_pool = tf.keras.layers.MaxPool1D(pool_size=2)(c_conv)
  c_conv = tf.keras.layers.Conv1D(4, 5, use_bias=False)(c_pool)
  c_pool = tf.keras.layers.MaxPool1D(pool_size=2)(c_conv)
  sigmoid = tf.keras.layers.Activation('sigmoid')(c_pool)

  # further define the MC-Model using Conv2D, Dense, Pooling and Flattening
  conv = tf.keras.layers.Conv1D(32, 5)(sigmoid)
  den = tf.keras.layers.Dense(732)(conv)
  flat = tf.keras.layers.Flatten()(den)
  den_1 = tf.keras.layers.Dense(10)(flat)
  drop = tf.keras.layers.Dropout(0.5)(den_1)
  out = tf.keras.layers.Dense(1, activation=tf.keras.activations.sigmoid)(drop)
  
  # Build the final model, show a summary and compile it w. metrics
  mc_cnn = tf.keras.Model(inputs=in_layer, outputs=out)
...
# for i in range(1):
	...
	for hfsep_dat, noise_dat, title in base_modalities_per_subject:
		...
		for data, labels, run_title in workload:
			...
			model = create_mc_cnn_for_1d(X_train.shape[1])

			history = model.fit(x=X_train, y=y_train, epochs=max_epochs, batch_size=32, validation_data=[X_eval, y_eval],
				callbacks=[
			...
			model.save('/media/christoph/Volume/paper/new_trainings/mc_cnn/models/%d_mc_cnn_on_%s' % (idx, run_title.replace('/', '-')))
