...
def load_model_and_predict(subject_id, sample_counter, modality_pair, rand_stat, true_labels_dict, predicted_labels_dict):
   ...
   try:
      ...
      y_pred = mc_cnn.predict(X_test)
      
      fpr_alexnet, tpf_alexnet, thresholds_alexnet = roc_curve(y_test, y_pred)
...
pl_a = '/media/christoph/Volume/Masterthesis/mc_cnn/models/%d/mc_cnn_on_k00%d-epoched_intrplt_filt_500_900_kx_%s'
pl_b = '/media/christoph/Volume/paper/new_trainings/mc_cnn/models/%d_mc_cnn_on_k00%d-epoched_intrplt_filt_500_900_kx_%s'
...
for subject_id in range(1, 11, 1):
   ...
   plt.title('MC-CNN ROC curve for SO%d' % subject_id) 
   plt.legend(loc='best')
   plt.gcf().set_size_inches(5.5, 4.75)   
   plt.xlim([0,1])  
   plt.ylim([0,1])  
   plt.savefig('/home/christoph/Desktop/paper_presentation/temp/meeting_20_04/mc_cnn_roc_s%d.png' % subject_id, dpi=200)
   plt.close('all')
